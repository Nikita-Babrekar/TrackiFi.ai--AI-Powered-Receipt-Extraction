...
# ==========================================
# 🛡️ BULLETPROOF PRODUCTION CORS CONFIGURATION
# ==========================================

# 1. Enable CORS broadly across all application routes
CORS(app, resources={r"/*": {"origins": "*"}}, supports_credentials=True)

# 2. Intercept preflight OPTIONS requests directly before they reach the routing logic
@app.before_request
...
# 3. Explicitly append security headers to all returning traffic profiles
@app.after_request
...
# ==========================================
# 🗄️ DATABASE CONNECTION PIPELINE
# ==========================================
...
# ==========================================
# ⚙️ MODEL DEFINITIONS & CLIENT INITIALIZATION
# ==========================================
...
if GEMINI_API_KEY:
    ...
    try:
        # Fallback to systemic context environment check
        client = genai.Client()
    except Exception as e:
        print(f"Warning: Default Gemini client initialization failed: {e}")
...
def upload_receipt():
    if not client:
        return jsonify({"error": "Gemini API client not initialized. Check your Railway variable keys."}), 500
    ...
    try:
        ...
        if not extracted_data:
            raise ValueError("The structured output could not be parsed correctly from the Gemini payload.")
